...
public class BookManagement
{
    public static void main(String[] args)
    {
        ...
        int bookCount = 0; //Dùng để đếm số lượng sách có trong thư viện
        int maxId = 1;//Dùng để gán bookId, tránh việc Id bị trùng lặp khi xóa sách và thêm mới
//        for (int i = 0; i < 5; i++) Test case
//        {
...
//        }
        while (true)
        {
            ...
            System.out.println("Nhập hành động muốn thực hiện theo các số từ 1-7");
            int choice = Integer.parseInt(scanner.nextLine());
            switch (choice)
            {
                case 1:
                    System.out.println("Hãy nhập số lượng sách mà bạn muốn thêm vào danh sách trong thư viện");
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        ...
                        Book newBook = new Book();
                        newBook.inputData(scanner);//Đã có validate qua hàm inputData nên không cần validate nữa
                ...
                case 3:
                    ...
                    for (int i = 0; i < bookCount; i++)
                    {//Bắt buộc phải dùng fori để tránh bị null pointer
                ...
                case 4:
                    ...
                    for (int i = 0; i < bookCount; i++)
                    {
                        if (deleteChoice == bookList[i].getBookId())
                        {
                            ...
                            {
                                bookList[j] = bookList[j + 1];//Shift element trong mảng về bên trái
                            }
                            bookList[bookCount - 1] = null;//Xóa sách ở vị trí cuối để không bị trùng
                            bookCount--;//Giảm số lượng đầu sách đi 1
                            break;
                        }
                    }
                    if (!bookExist)
